function saveBrainActivityForDifferenceGenre
    %add path
    addpath functions
    addpath niitools
    %define root
    [folder, ~, ~] = fileparts(which('saveBrainActivityForDifferenceGenre'));
    root = strcat(folder, '\..\');
    %make output folder
    [status, ~, ~] = mkdir(strcat(root,'output\phase3-3\'));
    assert(status == 1, 'output directory creation failed');
    %read zvalue for each run and genre
    [zvalue,label] = extractDifferenceOfGenresFeatures;
    %load a sample of nii file
    nii=  load_untouch_nii(strcat(root,'output\differenceGenre\run7\spmT_0004.nii'));
    for i = 1:10
        %overwrite sample data with zvalue
        nii.img = reshape(zvalue(i,:,:,:),[160 160 36]);
        ...
        f1.Position(3) = 1000;
        f1.Position(4) = 1000;
        all_fig_path = strcat(root, 'output\phase3-3\', label{i}, '.png');
